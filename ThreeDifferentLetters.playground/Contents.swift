...
func threeDifferentLetters(_ from:String, and:String)->Bool {
	
	guard from.count == and.count else { return false}
	var diffCounter = 0
	
	for char in from  {
		if !and.contains(char) {
			diffCounter += 1
...
threeDifferentLetters("Clamp", and: "Cramp")
threeDifferentLetters("Clamp", and: "Grans")
